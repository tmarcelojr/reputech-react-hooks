...
const LoginRegisterModal = (props) => {
  ...
  const login = async (values) => {
    ...
    try{
      const loginRes = await fetch('http://reputech-python.herokuapp.com/api/v1/users/login', {
        credentials: 'include',
        method: 'POST',
  ...
  const register = async (values) => {
    try {
      const registerRes = await fetch('http://reputech-python.herokuapp.com/api/v1/users/register', {
        credentials: 'include',
        method: 'POST',
        body: JSON.stringify(values),
        headers: {
